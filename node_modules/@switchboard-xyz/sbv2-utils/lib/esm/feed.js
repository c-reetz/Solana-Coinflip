...
import { AggregatorAccount, LeaseAccount, packInstructions, PermissionAccount, ProgramStateAccount, programWallet, signTransactions, SwitchboardDecimal, } from "@switchboard-xyz/switchboard-v2";
import Big from "big.js";
import { promiseWithTimeout } from "./async.js";
export async function awaitOpenRound(aggregatorAccount, queueAccount, payerTokenWallet, expectedValue = undefined, timeout = 30) {
    ...
    const updatedValuePromise = promiseWithTimeout(timeout * 1000, awaitUpdatePromise, new Error(`aggregator failed to update in ${timeout} seconds`)).finally(() => {
        if (accountWs) {
...
export async function createAggregator(program, queueAccount, params, jobs, fundLeaseAmount = new anchor.BN(0)) {
    const req = await createAggregatorReq(program, queueAccount, params, jobs, fundLeaseAmount);
    const { blockhash } = await program.provider.connection.getLatestBlockhash();
    const packedTxns = packInstructions(req.ixns, programWallet(program).publicKey, blockhash);
    const signedTxns = signTransactions(packedTxns, req.signers);
...
export async function createAggregatorReq(program, queueAccount, params, jobs, fundLeaseAmount = new anchor.BN(0), payerPubkey = programWallet(program).publicKey) {
    ...
    // Permission params
    const [permissionAccount, permissionBump] = PermissionAccount.fromSeed(program, queue.authority, queueAccount.publicKey, aggregatorKeypair.publicKey);
    // Lease params
