 ...
 * */
export const getDefaultProvider = (connection) => {
    return new anchor.AnchorProvider(connection, new AnchorWallet(DEFAULT_KEYPAIR), anchor.AnchorProvider.defaultOptions());
};
 ...
 */
export const getProgramDataAddress = (programId) => {
    return findProgramAddressSync([programId.toBytes()], new PublicKey("BPFLoaderUpgradeab1e11111111111111111111111"))[0];
};
/** Get the IDL address for a given programId
 * @param programId the programId for a given on-chain program
 * @return the publicKey of the IDL address
 */
export const getIdlAddress = async (programId) => {
    const base = (await PublicKey.findProgramAddress([], programId))[0];
    return PublicKey.createWithSeed(base, "anchor:idl", programId);
};
export const programHasPayer = (program) => {
    const payer = programWallet(program);
    return !payer.publicKey.equals(DEFAULT_KEYPAIR.publicKey);
};
export const getProgramPayer = (program) => {
...
export const verifyProgramHasPayer = (program) => {
    if (programHasPayer(program)) {
        return;
    }
    throw new NoPayerKeypairProvided();
};
export function getAnchorWalletPath(parsedToml) {
    let tomlData;
    if (parsedToml) {
        tomlData = parsedToml;
    }
...
export function getAnchorCluster(parsedToml) {
    let tomlData;
    if (parsedToml) {
        tomlData = parsedToml;
    }
...
export function loadPid(programKeypairPath) {
    if (!fs.existsSync(programKeypairPath)) {
        console.log(programKeypairPath);
        throw new Error(`Could not find keypair. Have you run 'anchor build'?`);
    }
    const programKeypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(programKeypairPath, "utf8"))));
    return programKeypair.publicKey;
}
export function getWorkspace(programName, programPath) {
    ...
    const cluster = tomlData.provider.cluster;
    const wallet = Keypair.fromSecretKey(Buffer.from(JSON.parse(fs.readFileSync(tomlData.provider.wallet, {
        encoding: "utf-8",
    }))));
    const programKeypairPath = path.join(programPath, `target/deploy/${programName.replace("-", "_")}-keypair.json`);
    let programId;
    switch (cluster) {
        case "localnet":
    ...
    const programIdlPath = path.join(programPath, `target/idl/${programName.replace("-", "_")}.json`);
    ...
    const provider = new anchor.AnchorProvider(new Connection(url, { commitment: "confirmed" }), new AnchorWallet(wallet), { commitment: "confirmed" });
