...
export class SwitchboardTestContext {
    ...
    static async getOrCreateSwitchboardWallet(program, mint, tokenAmount) {
        ...
        const finalBalance = await transferWrappedSol(program.provider.connection, payerKeypair, amountNeeded);
        return associatedTokenAccount.address;
    }
    ...
    async createStaticFeed(value, timeout = 30) {
        ...
        const aggregatorAccount = await createAggregator(this.program, this.queue, {
            batchSize: 1,
        ...
        const aggValue = await awaitOpenRound(aggregatorAccount, this.queue, this.payerTokenWallet, new Big(value), timeout);
    ...
    async updateStaticFeed(aggregatorAccount, value, timeout = 30) {
        ...
        const aggValue = await awaitOpenRound(aggregatorAccount, this.queue, this.payerTokenWallet, expectedValue, timeout);
    }
