...
import { CrankAccount, OracleAccount, PermissionAccount, ProgramStateAccount, } from "@switchboard-xyz/switchboard-v2";
...
export class SwitchboardTestEnvironment {
    constructor(ctx) {
        var _a;
    ...
    writeEnv(filePath) {
        ...
        console.log(`${chalk.green("Env File saved to:")} ${ENV_FILE_PATH.replace(process.cwd(), ".")}`);
    }
    writeJSON(outputDir) {
        const JSON_FILE_PATH = path.join(outputDir, "switchboard.json");
        fs.writeFileSync(JSON_FILE_PATH, JSON.stringify(this.toJSON(), (key, value) => {
    ...
    writeScripts(outputDir) {
        const LOCAL_VALIDATOR_SCRIPT = path.join(outputDir, "start-local-validator.sh");
        // create bash script to startup local validator with appropriate accounts cloned
        const baseValidatorCommand = `solana-test-validator -r --ledger .anchor/test-ledger --mint ${this.oracleAuthority.toBase58()} --bind-address 0.0.0.0 --url ${clusterApiUrl("devnet")} --rpc-port 8899 `;
        const cloneAccountsString = this.getAccountCloneString();
        const startValidatorCommand = `${baseValidatorCommand} ${cloneAccountsString}`;
        fs.writeFileSync(LOCAL_VALIDATOR_SCRIPT, `#!/bin/bash\n\nmkdir -p .anchor/test-ledger\n\n${startValidatorCommand}`);
        fs.chmodSync(LOCAL_VALIDATOR_SCRIPT, "755");
        console.log(`${chalk.green("Bash script saved to:")} ${LOCAL_VALIDATOR_SCRIPT.replace(process.cwd(), ".")}`);
        ...
        fs.writeFileSync(ORACLE_SCRIPT, `#!/usr/bin/env bash
...
docker-compose -f  "$script_dir"/docker-compose.switchboard.yml up
      `
        ...
        console.log(`${chalk.green("Bash script saved to:")} ${ORACLE_SCRIPT.replace(process.cwd(), ".")}`);
    }
    writeDockerCompose(outputDir) {
        const DOCKER_COMPOSE_FILEPATH = path.join(outputDir, "docker-compose.switchboard.yml");
...
`;
        ...
        console.log(`${chalk.green("Docker-Compose saved to:")} ${DOCKER_COMPOSE_FILEPATH.replace(process.cwd(), ".")}`);
    }
    writeAnchorToml(outputDir) {
        const ANCHOR_TOML_FILEPATH = path.join(outputDir, "Anchor.switchboard.toml");
...
`;
        ...
        console.log(`${chalk.green("Anchor.toml saved to:")} ${ANCHOR_TOML_FILEPATH.replace(process.cwd(), ".")}`);
    }
    ...
    static async create(payerKeypairPath, additionalClonedAccounts, alternateProgramId) {
        ...
        const payerKeypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(fs.readFileSync(fullKeypairPath, {
            encoding: "utf-8",
        }))));
        ...
        const programDataAddress = getProgramDataAddress(switchboardProgram.programId);
        const idlAddress = await getIdlAddress(switchboardProgram.programId);
        const queueResponse = await createQueue(switchboardProgram, {
            authority: payerKeypair.publicKey,
            name: "Test Queue",
            metadata: `created ${anchorBNtoDateString(new anchor.BN(Math.floor(Date.now() / 1000)))}`,
        ...
        const [programStateAccount, stateBump] = ProgramStateAccount.fromSeed(switchboardProgram);
        const programState = await programStateAccount.loadData();
        const mint = await queueAccount.loadMint();
        const payerSwitchboardWallet = await getOrCreateSwitchboardTokenAccount(switchboardProgram, mint);
        ...
        const [permissionAccount] = PermissionAccount.fromSeed(switchboardProgram, queue.authority, queueAccount.publicKey, oracleAccount.publicKey);
