...
import { clusterApiUrl, Connection, Keypair, PublicKey, sendAndConfirmTransaction, SystemProgram, SYSVAR_INSTRUCTIONS_PUBKEY, SYSVAR_RECENT_BLOCKHASHES_PUBKEY, Transaction, } from "@solana/web3.js";
 ...
 */
export const SBV2_DEVNET_PID = new PublicKey("2TfB33aLaneQb5TNVwyDz3jSZXS6jdW2ARw1Dgf84XCG");
/**
...
export const GOVERNANCE_PID = new PublicKey("GovER5Lthms3bLBqWub97yVrMmEogzX7xNjdXpPPCVZw"
 ...
 */
export function getSwitchboardPid(cluster) {
    switch (cluster) {
        case "devnet":
 ...
 */
export async function loadSwitchboardProgram(cluster, connection = new Connection(clusterApiUrl(cluster)), payerKeypair, confirmOptions = {
...
export const findAccountName = (program, accountInfo) => {
    const accountDiscriminator = accountInfo.data.slice(0, anchor.ACCOUNT_DISCRIMINATOR_SIZE);
    for (const accountDef of program.idl.accounts) {
        const typeDiscriminator = anchor.BorshAccountsCoder.accountDiscriminator(accountDef.name);
...
export function watchSwitchboardAccount(program, publicKey, accountName, callback) {
    ...
    return program.provider.connection.onAccountChange(publicKey, (accountInfo, context) => {
        const data = coder.decode(accountName, accountInfo === null || accountInfo === void 0 ? void 0 : accountInfo.data);
        callback(data);
    });
}
/**
...
export class ProgramStateAccount {
    ...
    async getTokenMint() {
        ...
        const switchTokenMint = spl.getMint(this.program.provider.connection, state.tokenMint);
    ...
    static async create(program, params) {
        ...
        if (params.mint === undefined) {
            const decimals = 9;
            mint = await spl.createMint(program.provider.connection, payerKeypair, payerKeypair.publicKey, null, decimals);
            const tokenVault = await spl.createAccount(program.provider.connection, payerKeypair, mint, Keypair.generate().publicKey);
            await spl.mintTo(program.provider.connection, payerKeypair, mint, tokenVault, payerKeypair.publicKey, 100000000);
            vault = tokenVault;
        }
        else {
            mint = params.mint;
            vault = await spl.createAccount(program.provider.connection, payerKeypair, mint, payerKeypair.publicKey);
        }
...
export class AggregatorAccount {
    ...
    static decode(program, accountInfo) {
        const coder = new anchor.BorshAccountsCoder(program.idl);
        const aggregator = coder.decode(AggregatorAccount.accountName, accountInfo === null || accountInfo === void 0 ? void 0 : accountInfo.data);
        return aggregator;
    }
    /**
    ...
    onChange(callback) {
        ...
        return this.program.provider.connection.onAccountChange(this.publicKey, (accountInfo, context) => {
            const aggregator = coder.decode(AggregatorAccount.accountName, accountInfo === null || accountInfo === void 0 ? void 0 : accountInfo.data);
            callback(aggregator);
        });
    }
    async onResult(callback) {
        return this.program.addEventListener("AggregatorValueUpdateEvent", (event, slot) => {
            const result = SwitchboardDecimal.from(event.value).toBig();
            const oracleValues = event.oracleValues.map((v) => SwitchboardDecimal.from(v).toBig());
            callback({
                feedPubkey: event.feedPubkey,
                result,
                slot: event.slot,
    ...
    async getLatestValue(aggregator, decimals = 20) {
        ...
        const mantissa = new Big(aggregator.latestConfirmedRound.result.mantissa.toString());
        const scale = aggregator.latestConfirmedRound.result.scale;
        const oldDp = Big.DP;
        Big.DP = decimals;
        const result = mantissa.div(new Big(10).pow(scale));
        Big.DP = oldDp;
        return result;
    }
    /**
    ...
    static async create(program, params) {
        ...
        await program.methods
            ...
            varianceThreshold: SwitchboardDecimal.fromBig(new Big((_e = params.varianceThreshold) !== null && _e !== void 0 ? _e : 0)),
    ...
    params) {
        ...
        const escrow = await spl.getAssociatedTokenAddress(params.tokenMint, leaseAccount.publicKey, true);
...
export class JobAccount {
    ...
    static decode(program, accountInfo) {
        const coder = new anchor.BorshAccountsCoder(program.idl);
        const data = coder.decode(JobAccount.accountName, accountInfo === null || accountInfo === void 0 ? void 0 : accountInfo.data);
        return data;
    }
    static decodeJob(program, accountInfo) {
        return protos.OracleJob.decodeDelimited(JobAccount.decode(program, accountInfo).data);
    }
}
...
export var SwitchboardPermissionValue;
...
})(SwitchboardPermissionValue || (SwitchboardPermissionValue = {}));
/**
...
export class PermissionAccount {
    ...
    async setVoterWeightTx(params) {
        ...
        const [addinState, _] = await PublicKey.findProgramAddress([Buffer.from("state")], params.addinProgram.programId);
        const [realmSpawnRecord] = anchor.utils.publicKey.findProgramAddressSync([Buffer.from("RealmSpawnRecord"), params.realm.toBytes()], params.addinProgram.programId);
        const [voterWeightRecord] = anchor.utils.publicKey.findProgramAddressSync([Buffer.from("VoterWeightRecord"), permissionData.grantee.toBytes()], params.addinProgram.programId);
...
export class OracleQueueAccount {
    ...
    async loadMint() {
        ...
        let mintKey = (_a = queue.mint) !== null && _a !== void 0 ? _a : PublicKey.default;
    ...
    static async create(program, params) {
        ...
        await program.methods
            ...
            varianceToleranceMultiplier: SwitchboardDecimal.fromBig(new Big((_j = params.varianceToleranceMultiplier) !== null && _j !== void 0 ? _j : 2)),
            authority: params.authority,
            ...
            enableBufferRelayers: (_p = params.enableBufferRelayers) !== null && _p !== void 0 ? _p : false,
        })
            .accounts({
...
OracleQueueAccount.accountName = "OracleQueueAccountData";
/**
...
export class LeaseAccount {
    ...
    static async create(program, params) {
        ...
        const escrow = await spl.getAssociatedTokenAddress(switchTokenMint.address, leaseAccount.publicKey, true);
        ...
        for (const idx in jobAccountDatas) {
            ...
            const authority = (_a = jobAccountData.authority) !== null && _a !== void 0 ? _a : PublicKey.default;
        ...
        await program.methods
            ...
            stateBump,
            leaseBump,
            withdrawAuthority: (_b = params.withdrawAuthority) !== null && _b !== void 0 ? _b : PublicKey.default,
            ...
            .preInstructions([
            spl.createAssociatedTokenAccountInstruction(payerKeypair.publicKey, escrow, leaseAccount.publicKey, switchTokenMint.address),
        ])
    ...
    async getBalance() {
        ...
        const escrow = await spl.getAccount(this.program.provider.connection, lease.escrow);
...
export class CrankAccount {
    ...
    async pop(params) {
        const payerKeypair = programWallet(this.program);
        return sendAndConfirmTransaction(this.program.provider.connection, await this.popTxn(params), [payerKeypair]);
    }
    /**
...
export class OracleAccount {
    ...
    static async create(program, params) {
        ...
        const [oracleAccount, oracleBump] = OracleAccount.fromSeed(program, params.queueAccount, walletKeypair.publicKey);
        const tokenRent = await program.provider.connection.getMinimumBalanceForRentExemption(spl.ACCOUNT_SIZE);
        await program.methods
            .oracleInit({
            name: ((_b = params.name) !== null && _b !== void 0 ? _b : Buffer.from("")).slice(0, 32),
            ...
            spl.createInitializeAccountInstruction(walletKeypair.publicKey, mint.address, programWallet(program).publicKey),
            spl.createSetAuthorityInstruction(walletKeypair.publicKey, programWallet(program).publicKey, spl.AuthorityType.AccountOwner, programStateAccount.publicKey, [programWallet(program), walletKeypair], spl.TOKEN_PROGRAM_ID),
        ])
    ...
    async getBalance() {
        const oracle = await this.loadData();
        const escrow = await spl.getAccount(this.program.provider.connection, oracle.tokenAccount);
        return Number(escrow.amount);
    }
}
...
export class VrfAccount {
    ...
    onChange(callback) {
        const coder = new anchor.BorshAccountsCoder(this.program.idl);
        return this.program.provider.connection.onAccountChange(this.publicKey, (accountInfo, context) => {
            const vrf = coder.decode(VrfAccount.accountName, accountInfo === null || accountInfo === void 0 ? void 0 : accountInfo.data);
            callback(vrf);
        });
    }
    /**
    ...
    static async create(program, params) {
        ...
        const escrow = await spl.getAssociatedTokenAddress(switchTokenMint.address, keypair.publicKey, true);
        await program.methods
            .vrfInit({
            stateBump,
            ...
            .preInstructions([
            spl.createAssociatedTokenAccountInstruction(payerKeypair.publicKey, escrow, keypair.publicKey, switchTokenMint.address),
            spl.createSetAuthorityInstruction(escrow, keypair.publicKey, spl.AuthorityType.AccountOwner, programStateAccount.publicKey, [payerKeypair, keypair]),
            anchor.web3.SystemProgram.createAccount({
                fromPubkey: payerKeypair.publicKey,
...
export class BufferRelayerAccount {
    ...
    static async create(program, params) {
        ...
        const escrow = await spl.getAssociatedTokenAddress(switchTokenMint.address, keypair.publicKey);
        const size = 2048;
    ...
    async openRound() {
        ...
        const switchTokenMint = await queueAccount.loadMint();
        const source = (await spl.getOrCreateAssociatedTokenAccount(this.program.provider.connection, programWallet(this.program), switchTokenMint.address, programWallet(this.program).publicKey, true)).address;
        ...
        const transferIx = spl.createTransferInstruction(source, escrow, programWallet(this.program).publicKey, queueData.reward.toNumber());
...
export async function sendAll(provider, reqs, signers, skipPreflight) {
    const res = [];
    try {
 ...
 * @returns Transaction[]
 */
export function packInstructions(instructions, feePayer = PublicKey.default, recentBlockhash = PublicKey.default.toBase58()) {
    const packed = [];
 ...
 */
export async function packTransactions(connection, transactions, signers, feePayer) {
 ...
 */
export function signTransactions(transactions, signers) {
    ...
    for (const transaction of transactions) {
        ...
        const currentSigners = signers.filter((signer) => Boolean(sigsNeeded.find((sig) => sig.equals(signer.publicKey))));
...
export function programWallet(program) {
