...
declare_id!("86kL1NMhYsMedrYmUbYa2XPWQFNNzEoPjEZQzxjgARw7");
...
pub mod anchor_vrf_parser {
    ...
    pub fn init_gambling_state(ctx: Context<InitGamblingState>) -> Result<()> {
        InitGamblingState::init_gstate(ctx)
    }

    pub fn start_gamble(ctx: Context<StartGamble>, amount: u64) -> Result<()> {
        StartGamble::transfer_to_house(ctx, amount)
    }

    pub fn heads(ctx: Context<BetHeads>) -> Result<()> {
        BetHeads::heads(ctx)
    }

    pub fn tails(ctx: Context<BetTails>) -> Result<()> {
        BetTails::tails(ctx)
    }
